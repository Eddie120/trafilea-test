...
func (o *CreateCartURL) Build() (*url.URL, error) {
	...
	var _path = "/users/{userId}/carts/"

	userID := o.UserID
	if userID != "" {
		_path = strings.Replace(_path, "{userId}", userID, -1)
	} else {
		return nil, errors.New("userId is required on CreateCartURL")
	}
...
func (o *CreateCartURL) BuildFull(scheme, host string) (*url.URL, error) {
	if scheme == "" {
		return nil, errors.New("scheme is required for a full url on CreateCartURL")
	}
	if host == "" {
		return nil, errors.New("host is required for a full url on CreateCartURL")
	}
